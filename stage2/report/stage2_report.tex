...
No single free provider covers both series: CBOE publishes the complete VIX history at
no charge but does not publish S\&P~500 index history. \citet{fernandes2014} state no
data source at all, and \citet{wen2024} use OptionMetrics IvyDB and Tonghuashun---both
paid subscriptions, and both options databases rather than index sources.
\citet{liu2015} face the same constraint, take both series from Yahoo Finance, and
report that comparing Yahoo's VIX against CBOE's shows ``only very minor differences for
a few days.'' Accordingly the VIX is taken from CBOE's official file and the S\&P~500
from Yahoo Finance (\texttt{\^{}GSPC}); the Yahoo VIX was also downloaded, solely to
replicate that validation on the present sample.

Across the 3,681 common dates the two VIX providers agree exactly, to two decimal
places, on \textbf{99.76\%} of observations; nine days differ by more than 0.01 and the
correlation is 0.99997, reproducing the conclusion of \citet{liu2015} on more recent
data. The one material exception is 6~February~2026, where Yahoo reports 20.37 against
CBOE's 17.76. The S\&P~500 rose 1.95\% that day and CBOE's path
($21.77 \rightarrow 17.76 \rightarrow 17.36$) is consistent with the rally while Yahoo's
is not, so the CBOE series is used throughout.

The calendars were compared before merging rather than letting an inner join discard
mismatches silently. CBOE reports a VIX close on 32 dates on which the NYSE cash market
was closed---US market holidays, concentrated from 2022 onward. These are genuine index
values, since VIX is calculated from SPX option quotes disseminated in sessions that need
not coincide with equity trading hours, but no matched return can be formed on those days
and they are excluded. Neither series contains internal missing values or duplicate
dates, and the largest gap between consecutive observations is the five-day NYSE closure
for Hurricane Sandy ending 31~October~2012, left unimputed. Returns are computed
as $r_t = 100\,[\ln(P_t) - \ln(P_{t-1})]$ on the \emph{full} price history before the
sample is truncated, so the first in-sample return is measured against the
30~December~2011 close rather than being undefined.
...
conditions hold. $\omega = 0.0431$ is the constant floor of the variance equation;
combined with the persistence it implies a long-run daily variance of
$\omega/(1-\alpha-\beta) = 1.010$ and an annualised long-run volatility of 15.95, close
to the sample standard deviation of returns as it should be. $\alpha = 0.167$ is the
reaction coefficient---an unexpected move raises today's variance by 16.7\% of the
squared surprise---while $\beta = 0.791$ is the carry-over from yesterday's variance,
the most precisely estimated parameter in the model ($t = 30.9$) and the one that
...
10--13\% below afterwards, interpreting precisely that shortfall as a variance risk
premium; the 22\% found here on 2012--2026 data falls in the same range. The exception is
instructive: GARCH exceeds VIX on 13.3\% of days, and the largest overshoot---44 points
on 17~March~2020---follows directly from the $-12.77\%$ return of 16~March, since
$\sigma_t^2$ depends on $\epsilon_{t-1}^2$ and a single extreme return is mechanically
propagated into the next day's variance. Annualised GARCH volatility reached 120 while
VIX, already pricing an eventual return to calm, stood at 75.9.
...
\noindent
The correlation is near zero contemporaneously and $+0.41$ at a one-day lag. The reason
is structural: $\sigma_t$ is a function of information dated $t-1$ and cannot respond to
today's news until tomorrow, whereas VIX is priced off current option quotes and responds
immediately. The two measures agree about the \emph{level} of volatility but learn about
\emph{changes} one day apart. This bears directly on the project: a predictor that merely
tracked VIX would be redundant, whereas GARCH volatility is strongly related in levels,
reacts on a different schedule, and differs by a systematic risk-premium wedge that VIX
contains by construction and GARCH cannot. Whether that survives as \emph{incremental}
predictive power once VIX's own history is conditioned upon is the question for the
forecasting stage, and the ACF result above shows the own-history baseline to be a
demanding one.

\section{Conclusions}

\textbf{Is VIX highly persistent?} Yes. First-order autocorrelation is 0.961, still
0.563 after one month and 0.337 after a quarter, with the ACF outside the white-noise
band as far as lag 250. Decay is hyperbolic rather than geometric. VIX is nonetheless
stationary ($\text{ADF} = -5.560$, $p = 1.5\times10^{-6}$).

\textbf{Is there volatility clustering in S\&P 500 returns?} Yes, unambiguously. Raw
returns are near-unforecastable, but squared returns are autocorrelated at 0.449 at lag~1
and remain positive beyond lag~20. Ljung--Box gives $Q(10) = 3{,}955$ on squared returns
against 236 on raw returns, and ARCH-LM returns $\text{LM} = 1{,}109$ with
$p \approx 10^{-237}$.

\textbf{Is GARCH volatility highly persistent?} Yes. $\alpha+\beta = 0.9573$ implies a
half-life of 15.9 trading days. The process is stationary, with a long-run annualised
volatility of 15.95. Under Student-$t$ errors persistence rises to 0.984.
...
$+0.410$ at one day shows that GARCH responds to the same information one day later,
because $\sigma_t$ conditions on data through $t-1$ while VIX is priced off current
quotes.

\textbf{What is the main difference between VIX and GARCH volatility?} They are
different objects rather than competing estimates of the same one. VIX is
forward-looking, extracted from option prices, and expresses the market's expectation of
volatility over the coming 30 calendar days, whereas GARCH volatility is backward-looking
and computed mechanically from the history of realised returns. VIX is risk-neutral and
therefore includes a variance risk premium, evident in the 22\% average wedge; GARCH is a
physical-measure forecast and includes none. VIX also reprices within the trading day
while GARCH cannot move until the day after a shock---the source of both the one-day lag
in the change correlation and the March~2020 episode, in which GARCH extrapolated the
crash to an annualised 120 while VIX peaked at 82.7.
